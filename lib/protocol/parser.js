...
var stream = require('stream')
  , util = require('util')
  , debug = require('debug')('messaging:Parser')
...
ProtocolParser.prototype._transform = function _transform(chunk, encoding, callback) {
  debug('transform[%s desired, %s backlog]:', this._desired, this._backlog.length, chunk)
...
ProtocolParser.prototype._pushChunk = function _pushChunk(chunk, callback) {
  ...
  debug('push[%s desired, %s backlog]:', this._desired, this._backlog.length, chunk)
...
ProtocolParser.prototype._pushCompleteFrame = function _pushCompleteFrame(chunk) {
  ...
  debug('Parsed frame:', chunk)

  if (this._moreFramesDesired) {
    return
  }

  debug('Parsed message:', this._messageBacklog)

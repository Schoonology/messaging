...
/**
 * Calls `fn(element)` once for each element in `arr`, starting with the "next"
 * element in `arr` as defined by `common.nextRoundRobin`. If `fn` returns
 * a truthy value, the iteration stops and this function returns `true`.
 * Otherwise, iteration continues until `arr` is exhausted and this function
 * returns `false`.
 *
 * See `common.nextRoundRobin` for other caveats.
 */
common.someRoundRobin = function someRoundRobin(arr, fn) {
  for (var i = 0; i < arr.length; i++) {
    if (fn(common.nextRoundRobin(arr))) {
      return true
    }
  }

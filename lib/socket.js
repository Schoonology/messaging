...
var stream = require('stream')
  , url = require('url')
  , debug = require('debug')('messaging:Socket')
  , mi = require('mi')
...
mi.inherit(Socket, stream.Duplex)
...
Socket.prototype.bind = function bind(endpoint) {
  ...
  server.on('listening', function () {
    debug('Listening on %s.', server.endpoint)
...
Socket.prototype.connect = function connect(endpoint) {
  ...
  debug('Connecting to %s...', client.endpoint)

  self._clients.push(client)

  client.setNoDelay(true)
  client.on('connect', function () {
    debug('Connected to %s.', client.endpoint)
  ...
  client.on('error', function (err) {
    // TODO(schoon) - The caller will want to do any number of
    // application-specific things based on this error, which should be mapped
    // to a more relevant error: Have we lost any messages? Is there a bad
    // peer connection? Should we apply backpressure?
    self.emit('error', err)
  })
...
Socket.prototype._fairRead = function _fairRead() {
  ...
  debug('Reading fairly from %s peers.', self._queue.length)
...
Socket.prototype.close = function close() {
  ...
  // TODO(schoon) - Log another message and emit 'close' and/or 'end' when
  // all connections have been closed successfully.
  debug('Closing...')
